...
global_conncetion = mysql.connector.connect(**conn_dict)
...
def get_cursor():
    ...
    if global_conncetion.is_connected():
        ...
        return global_conncetion.cursor()
    else:
        print("opening a new connection")
        global_conncetion = mysql.connector.connect(**bot_functions.conn_dict)
        return global_conncetion.cursor()
...
async def tim_start(user_id, server_id, channel_id, timer_duration, break_duration, timer_type, message = None):
    ...
    #with bot_functions.get_cursor() as cursor:
        start = time.time()
        cursor = get_cursor()
...
async def timer_cancel(message, user_id, server_id, save = True):
    ...
    #with bot_functions.get_cursor() as cursor:
        ...
        cursor.execute(f"select * from timers where user_id = {user_id} and server_id = {server_id}")
...
async def leaderboard(server_id, channel):
    ...
    cursor.execute(f"""select * from s{server_id} order by work_time + study_time desc""")
...
async def on_message(message):
    ...
    if valid_command:
        ...
        if len(command) == 1 and command[0].lower() == "study":
            ...
            rtime_timer_list = cursor.fetchall()
